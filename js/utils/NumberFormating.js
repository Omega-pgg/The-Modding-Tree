...
function regularFormat(num, precision) {
    ...
    if (num.mag < 0.1 && precision !==0) precision = Math.max(precision, 3)
...
function t4format(x,m) {
    ...
    if (x>20) t4f = t4tens[Math.floor(x/10)%10]+t4ones[x%10]
	return t4f
}
...
function standard(decimal, precision){
	...
	if (decimal.layer > 4 && decimal.mag>=0 || (decimal.mag >= Math.log10(3e90) && decimal.layer == 4)) {
...
function formatSciEng(decimal, precision) {
	...
	if (player.notation == 'Mixed Scientific' || player.notation == 'Mixed Engineering'){
		if (decimal.layer < 1 || (Math.abs(decimal.mag) < 303 && decimal.layer == 1)) return standard(decimal,precision)
	}
